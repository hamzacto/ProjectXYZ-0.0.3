    ...
    def _hubspot_deal_loader(
        ...
        try:
            with Session(engine) as db:
                ...
                if query:
                    logger.info(f"Filtering deals with query: {query}")
                    
                    # Check if the query is a numeric ID
                    if query.isdigit():
                        logger.info(f"Query appears to be a deal ID: {query}")
                        # Use a direct GET request to fetch by ID instead of search
                        deal_id_url = f"https://api.hubapi.com/crm/v3/objects/deals/{query}"
                        params = {
                            "properties": "dealname,amount,dealstage,pipeline,closedate,hubspot_owner_id,createdate,hs_lastmodifieddate",
                            "associations": "company,contact"
                        }
                        response = requests.get(deal_id_url, headers=headers, params=params)
                        ...
                        if response.status_code == 200:
                            ...
                            return [Data(text=f"No deal found with ID: {query}")]
                        else:
                            error_message = f"Failed to retrieve deal by ID: {response.text}"
                            logger.error(error_message)
                            return [Data(text=error_message)]
